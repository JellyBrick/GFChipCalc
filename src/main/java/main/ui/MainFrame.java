...
public class MainFrame extends JFrame {
    ...
    private ShortcutKeyAdapter initSKA_focusable() {
        ...
        ska.addShortcut_c(KeyEvent.VK_D, this::display_toggleType);
        ...
        ska.addShortcut(KeyEvent.VK_F5, this::process_toggleStartPause);
        ska.addShortcut(KeyEvent.VK_F6, this::process_stop);
    ...
    private ShortcutKeyAdapter initSKA_pi() {
        ...
        ska.addShortcut_c(KeyEvent.VK_N, this::invFile_new);
        ska.addShortcut_c(KeyEvent.VK_O, this::invFile_open);
        ska.addShortcut_c(KeyEvent.VK_S, this::invFile_save);

        ska.addShortcut_cs(KeyEvent.VK_S, this::invFile_saveAs);

        ska.addShortcut(KeyEvent.VK_ENTER, this::invStat_focusNextStat);
    ...
    private ShortcutKeyAdapter initSKA_pool() {
        ...
        ska.addShortcut(KeyEvent.VK_C, this::pool_cycleColor);
        ska.addShortcut(KeyEvent.VK_R, this::pool_toggleOrder);
        ska.addShortcut(KeyEvent.VK_SPACE, this::pool_addToInv);
    ...
    private ShortcutKeyAdapter initSKA_inv() {
        ...
        ska.addShortcut(KeyEvent.VK_C, this::invStat_cycleColor);
        ska.addShortcut(KeyEvent.VK_M, this::invStat_toggleMarked);
        ska.addShortcut(KeyEvent.VK_T, this::invStat_openTagDialog);
        ska.addShortcut(KeyEvent.VK_R, this::display_toggleOrder);
        ska.addShortcut(KeyEvent.VK_DELETE, this::invStat_delete);
        ska.addShortcut(KeyEvent.VK_MINUS, this::invStat_decLevel);
        ska.addShortcut(KeyEvent.VK_SUBTRACT, this::invStat_decLevel);
        ska.addShortcut(KeyEvent.VK_EQUALS, this::invStat_incLevel);
        ska.addShortcut(KeyEvent.VK_ADD, this::invStat_incLevel);
    ...
    private ShortcutKeyAdapter initSKA_comb() {
        ...
        ska.addShortcut_c(KeyEvent.VK_O, this::progFile_open);
        ska.addShortcut_c(KeyEvent.VK_S, this::progFile_saveAs);

        ska.addShortcut(KeyEvent.VK_C, this::comb_nextBoardName);
    ...
    public void invStat_loadStats() {
        if (!invStat_loading) {
            ...
            invMarkCheckBox.setSelected(singleSelected && invList.getSelectedValue().isMarked());
    ...
    private void invStat_setTagButtonText() {
        ...
        if (invList.getSelectedIndices().length >= 1) {
            ...
            for (Tag t : tags) {
                ...
                widthStr.append(t.getName()).append(", ");
            }
    ...
    private void invStat_refreshStatComboBoxes() {
        if (!invStat_loading) {
            invStat_loading = true;
            invComboBoxes.forEach(JComboBox::removeAllItems);
    ...
    private void invStat_readInput(int number) {
        if (invList.getSelectedIndices().length == 1) {
            ...
            for (int i = 0; i < inputs.length; i++) {
                if (statInputBuffer.size() >= i + 1) {
                    ...
                    for (int j = i; j < statInputBuffer.size(); j++) {
                        t.append(statInputBuffer.get(j));
                    }
                    inputs[i] = t.toString();
                }
            }
            ...
            if (app.setting.displayType == DISPLAY_STAT) {
                ...
                int pt = Integer.parseInt(inputs[inputs.length - 1]);
                if (pt < nItems) {
    ...
    private void invStat_delete() {
        if (invList.getSelectedIndices().length >= 1) {
            ...
            indexList.forEach(this::inv_chipsRemove);
            display_refreshInvListCountText();
            invStat_enableSave();
        }
    }
    ...
    public void display_applyFilterSort() {
        ...
        switch (invSortTypeComboBox.getSelectedIndex()) {
            case SORT_SIZE:
                temp.sort(Chip::compare);
                break;
            case SORT_LEVEL:
                temp.sort(Chip::compareLevel);
                break;
            case SORT_STAR:
                temp.sort(Chip::compareStar);
                break;
            case SORT_DMG:
                temp.sort((c1, c2)
    ...
    private void process_start() {
        ...
        if (start) {
            ...
            if (app.setting.maxLevel) {
                candidates.forEach(Chip::setMaxLevel);
            }
        ...
        SwingUtilities.invokeLater(() -> {
            try {
                if (forceUpdate || assembler.boardsUpdated()) {
                    ...
                    combLM.clear();
                    ar.boards.forEach(combLM::addElement);

                    combFreqLM.clear();
                    ar.freqs.forEach(combFreqLM::addElement);
    ...
    private void invFile_openImageDialog() {
        ImageDialog.getData(app).forEach(this::inv_chipsAdd);
    }
    ...
    private void initComponents() {
        ...
        combInfoPanelLayout.linkSize(javax.swing.SwingConstants.VERTICAL, ticketLabel, xpLabel);
        ...
        poolControlPanelLayout.linkSize(javax.swing.SwingConstants.HORIZONTAL, poolRotLButton, poolRotRButton, poolSortButton);
        ...
        poolControlPanelLayout.linkSize(javax.swing.SwingConstants.VERTICAL, poolRotLButton, poolRotRButton, poolSortButton);
        ...
        jPanel7Layout.linkSize(javax.swing.SwingConstants.HORIZONTAL, invDelButton, invRotLButton, invRotRButton);
        ...
        jPanel7Layout.linkSize(javax.swing.SwingConstants.VERTICAL, invDelButton, invRotLButton, invRotRButton);
        ...
        combRTPanelLayout.linkSize(javax.swing.SwingConstants.HORIZONTAL, boardImageLabel, loadingLabel);
