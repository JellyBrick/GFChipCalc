...
public class AppText {
    ...
    public static final Map<Integer, String> TEXT_MAP_COLOR = new HashMap<>() // <editor-fold defaultstate="collapsed">
    {
        {
    ...
    private static final Map<Locale, Properties> LANGMAP = new HashMap<>() // <editor-fold defaultstate="collapsed">
    {
        {
    ...
    private static final List<String> KEYS_HTML_CENTER = Collections.singletonList(
            COMB_DESC
    );
    ...
    public String getText(Locale locale, String key, String... replaces) {
        ...
        try {
            if (EN_US.equals(locale) && KEYS_PLURAL.contains(key) && Integer.parseInt(replaces[0]) > 1) {
                value += "s";
            }
        } catch (Exception ignored) {
        }
