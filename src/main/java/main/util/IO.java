...
public class IO {
    ...
    private static List<Board> loadCombination(@NotNull String s, @NotNull Iterator<String> bri) {
        ...
        if (v.isCurrent(4, 0, 0)) {
            if (bri.hasNext()) {
                ...
                String name = info[0];
                int star = Integer.parseInt(info[1]);
                ...
                while (bri.hasNext()) {
                    // Chips
                    int nChip = Integer.parseInt(bri.next());
                    ...
                    chips.forEach(Chip::setMaxLevel);

                    // Rotations
                    String[] rotStrs = bri.next().split(",");
                    for (int i = 0; i < nChip; i++) {
                        int r = Integer.parseInt(rotStrs[i]);
                        if (!v.isCurrent(4, 2, 0)) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(fileName))) {
            ...
            if (bri.hasNext()) {
                ...
                if (v.isCurrent(5, 3, 0)) {
                    ...
                    boards.forEach((b) -> b.forEachChip(chipSet::add));
    ...
    public static void exportProps(@NotNull App app, Component component) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(component, AppText.DISPLAY_EXPORT_FAIL_BODY, AppText.DISPLAY_EXPORT_FAIL_TITLE, JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public static Stat parseStat(@NotNull String s) {
        ...
        int hit = d.length > 2 ? Integer.parseInt(d[2]) : 0;
        int rld = d.length > 3 ? Integer.parseInt(d[3]) : 0;
    ...
    public static BoardSetting parseBS(@NotNull List<String> data, boolean advancedSetting) {
        ...
        data.stream().map((line) -> line.split(";")).forEachOrdered((parts) -> {
            ...
            if (Arrays.asList(Board.NAMES).contains(name)) {
                ...
                if (parts.length > 5) {
                    out.setPresetIndex(name, star, Integer.parseInt(parts[5]));
                }
            }
        });
        return out;
    ...
    public static Chip parseChip(@NotNull Version3 v, @NotNull String s, @NotNull Set<Tag> tagPool) {
        ...
        int color = Integer.parseInt(it.next());

        Stat pt = parseStat(it.next());

        int initLevel = Integer.parseInt(it.next());
        int initRotation = Integer.parseInt(it.next());
    ...
    public static ProgressFile parseProgressFile(@NotNull Version3 v, @NotNull Iterator<String> it, @NotNull List<Chip> invChips) {
        ...
        if (!v.isCurrent(6, 5, 3) && calcMode == CalcExtraSetting.CALCMODE_FINISHED) {
            ...
            if (v.isCurrent(6, 4, 0)) {
                nComb = Integer.parseInt(it.next());
            }
        ...
        int markMin = Integer.parseInt(it.next());
        int markMax = Integer.parseInt(it.next());
        int markType = Integer.parseInt(it.next());
        int sortType = Integer.parseInt(it.next());

        Stat stat = parseStat(it.next());
        Stat pt = parseStat(it.next());

        int nComb = Integer.parseInt(it.next());
        int progress = Integer.parseInt(it.next());
        int progMax = Integer.parseInt(it.next());
        int tag = Integer.parseInt(it.next());
