...
public class ProxyHandlerThread extends Thread {
    ...
    private static final Map<Integer, String> STATUS = new HashMap<>() // <editor-fold defaultstate="collapsed">
    {
        {
            put(404, "Not Found");
    ...
    private void handleResponse(@NotNull HttpURLConnection connection) throws IOException {
        ...
        try (DataOutputStream ptcOS = getDOS(cpSocket.getOutputStream())) {
            ...
            // Body
            boolean bodyTE = pm.containsResHeader(Proxy.TRANSFER_ENCODING) && "chunked".equalsIgnoreCase(pm.getResHeader(Proxy.TRANSFER_ENCODING));
